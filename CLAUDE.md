...
- **エラーハンドリング**: バルクインポートで拒否されたドキュメントは件数を数え、
  最後に戻り値のエラーとして返す。処理自体は続行して入る分は入れ切る

### import のエラー処理で守るべきこと

- **bulk のレスポンスは `Failed()` まで見る**

  `BulkService.Do` が返すエラーは HTTP や接続の失敗しか表しません。
  マッピング不一致のような個々のドキュメントの拒否はレスポンスの items に
  入るため、`Do` の戻り値だけを見ていると 1 件も入らなくても成功になります。
  実際に全件拒否のダンプで `finish import row count 2` と報告して exit 0 で
  終わっていました。件数を報告する分、ログ上は成功したように読めます。

- **拒否があれば戻り値で失敗を返す**

  拒否されたドキュメントを数え、1 件でもあれば `ImportData` はエラーを
  返します。cron や CI から取りこぼしに気づけるようにするためです。
  ログ出力だけでは、監視していなければ気づけません。

- **拒否されても処理は続行する**

  不正な行が混ざっていても、入れられる分は入れ切ります。1 件の失敗で
  全体を捨てるほうが実害が大きいためです。中断するのはリクエスト自体が
  失敗した場合 (接続断など) だけです。以降も失敗し続けるためです。

- **JSON の解釈に失敗したら行番号を添える**

  大きなダンプで「どこかが壊れている」とだけ言われても、原因の行を
  探せません。
